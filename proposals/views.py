...
def proposal_submission(request):
    ...
    if request.method == 'POST':
        ...
        if title and proposal_content and category:
            Proposal.objects.create(title=title, content=proposal_content, category=category)
            return redirect('index')
        else:
            return HttpResponseBadRequest("Title, proposal content, and category are required.")
    else:
